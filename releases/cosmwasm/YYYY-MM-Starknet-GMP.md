...
- [VotingVerifier v1.1.1](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/voting-verifier-v1.1.0)
- [Gateway vX.X.X](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/gateway-v1.1.1)
- [MultisigProver vX.X.X](https://github.com/axelarnetwork/axelar-amplifier/releases/tag/multisig-prover-v1.1.1)

## Background

These are the instructions for deploying Amplifier contracts for `Starknet` connection.

### Pre-requisites

Deploy the [External Gateway](../../starknet/docs/gateway.md) address, as `VotingVerifier` deployment requires the `sourceGatewayAddress` which is the External Gateway address.
...
"$CHAIN" : {
  ...
  "blockExpiry": 10,
  "confirmationHeight": 1000000, # Starknet uses a custom finality mechanism, so we set this value very high (i.e. 1000000) to prevent accidental use
...
11. Update `ampd` with the Starknet chain configuration. Verifiers should use their own Starknet RPC node for the `http_url` in production.


| Network              | `http_url`        |
| -------------------- | ----------------- |
| **Devnet-amplifier** | [starknet testnet RPC URL] |
| **Stagenet**         | [starknet testnet RPC URL] |
| **Testnet**          | [starknet testnet RPC URL] |
| **Mainnet**          | [starknet mainnet RPC URL] |


```bash
[[handlers]]
chain_name="starknet-dev"
...
12. Update `ampd` with the Solana chain configuration.
